...
public class BenchmarkArrayObjects {
    ...
    public static void main(String[] args) {
        ...
        if (args.length != 3)
        {
            System.out.println("Please use the proper command line arguments\n" + 
                "Run this program like this: java BenchmarkArrayObjects <Array Size> <Number of Tests> <Number of Threads>");
            return;
        }
        ...
        Item[] array = new Item[ARRAY_SIZE];
        for (int i = 0; i < ARRAY_SIZE; i++)
            array[i] = new Item("Number " + i, random.nextDouble() * ARRAY_SIZE);
        ...
        for (int i = 0; i < NUM_TESTS; i++) 
        {
            ...
            Item[] standardArray = array.clone();
            // Run and time the sorting algorithm by passing a runnable lambda
            long standardTime = timeSort(() -> BenchmarkArrayObjects.mergeSort(standardArray));
            ...
            Item[] parallelArray = array.clone();
            long parallelTime = timeSort(() -> new MultiMerge<Item>().sort(parallelArray, NUM_THREADS));
