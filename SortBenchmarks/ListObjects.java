...
public class ListObjects 
{
    ...
    public static void main(String[] args) 
    {
        ...
        for (int i = 0; i < ARRAY_SIZE; i++)
            items.add(new Item("Number " + i, random.nextDouble() * ARRAY_SIZE));
        ...
        {
            // run single thread
            long[] singleThreadList = ListObjects.singleThreadMetric(items, ARRAY_SIZE);
            ...
            // run four thread
            long[] fourThreadList = ListObjects.nThreadMetric(items, 4);
            ...
            // run 8 thread
            long[] eightThreadList = ListObjects.nThreadMetric(items, 8);
            ...
            // run 16 thread
            long[] sixteenThreadList = ListObjects.nThreadMetric(items, 16);
    ...
    private static long[] singleThreadMetric(List<Item> items, int size)
    {
    ...
    private static long[] nThreadMetric(List<Item> items, int threads)
    {
        long beforeUsedMem = Runtime.getRuntime().totalMemory() - Runtime.getRuntime().freeMemory();
        long startTime = System.nanoTime();
